...
### Бот принимает только голосовые — другие форматы отклоняет

![Бот принимает только голосовые](screenshots/bot%20test%201.PNG)
...
- 🐛 Исправлен баг: `set_x()` в fpdf2 сбрасывается при `multi_cell()` — найдено обходное решение через явные пробелы в тексте
- ✨ PDF теперь форматирован как настоящий документ: красная строка в начале каждого абзаца, пустая строка между абзацами, выравнивание по левому краю
- 🔍 Выявлено ограничение Telegram: длинный текст не поддерживает визуальные отступы — пользователи направляются к PDF для удобного чтения
